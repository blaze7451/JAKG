...
class KG_graph():
    ...
    def draw_kg(self):
        ...
        edgeDic = {(e[0], e[1]): G.get_edge_data(*e)["edge"] for e in G.edges} 
        plt.figure(figsize=(12,12))
        node_sizes = [value*800 for _, value in G.degree()]
        pos = nx.spring_layout(G)
        kg_graph = nx.draw(G, with_labels=True, node_color='skyblue', edge_cmap=plt.cm.Blues, pos = pos, node_size=node_sizes, alpha=0.8, font_size=10)
        nx.draw_networkx_edge_labels(G, pos, edge_labels=edgeDic, label_pos=0.5)
